...
# app.add_middleware(
#     CORSMiddleware,
#     allow_origins=["http://localhost:*", "http://127.0.0.1:*"], # Use the URL where your React app runs
...
#             new_chat = Chat(title=f"Chat {utc_now.strftime('%Y-%m-%d %H:%M')}", messages=[])
#             print('#############################################################', new_chat)
...
#                 "prompt": full_prompt,
#                 "stream": False,
#                 "format": "json"  # Added: Enforce JSON format to avoid whitespace issues
#             }
...
#             try:
#                 ai_response_json = json.loads(result.get("response", "{}"))
...
from sqlalchemy.orm import Session
from pydantic import BaseModel
from sqlalchemy.orm.attributes import flag_modified  # Correct import for marking JSON mutations
...
# Enable CORS for local development (adjusted to common React ports)
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://localhost:5173",
...
class LessonCreate(BaseModel):
    title: str
    content: Dict[str, Any]  # The generated JSON
...
def play(req: PlayRequest, db: Session = Depends(get_db)):
    ...
    chosen_option = db.query(models.Option).filter(
        models.Option.id == req.option_id,
        models.Option.scenario_id == scenario.id
    ).first()
    if not chosen_option:
        raise HTTPException(status_code=404, detail="Option not found")

    all_options = db.query(models.Option).filter(models.Option.scenario_id == scenario.id).all()
    min_co2e = min(opt.co2_impact for opt in all_options)
    max_co2e = max(opt.co2_impact for opt in all_options)

    saved = max(0, max_co2e - min_co2e)
    actually_saved = max(0, max_co2e - chosen_option.co2_impact)

    # Save play history (assuming user_id=1 for demo; integrate auth for real use)
    play_record = models.Choice(
    ...
    if chosen_option.co2_impact == min_co2e:
        message = f"You used {chosen_option.co2_impact:.2f} kg CO₂e. and saved {actually_saved:.2f} kg CO₂e today by making the best choice!"
    else:
        message = f"You used {chosen_option.co2_impact:.2f} kg CO₂e. The best choice could have saved you {saved:.2f} kg CO₂e."
